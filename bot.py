...
def passes_filters(dex: dict, pump, holders: dict, fees: float) -> tuple:
    ...
    # 1h vol should not exceed 3x the MC — if it does, volume is fake
    vol_1h = dex["vol_1h"]
    if mc and vol_1h and vol_1h > mc * 3:
        return False, f"Suspicious vol — 1h vol ${fmt(vol_1h)} is {vol_1h/mc:.1f}x the MC (wash trading)"

    # buy/sell ratio must be between 0.3 and 5.0
    # perfectly equal (bots) or wildly one-sided both get rejected
    buys  = dex["buys_5m"]
    sells = dex["sells_5m"]
    if buys + sells > 0:
        ratio = buys / max(sells, 1)
        if ratio < 0.3:
            return False, f"B/S ratio {ratio:.2f} — heavy sell pressure / distribution"
        if ratio > 5.0:
