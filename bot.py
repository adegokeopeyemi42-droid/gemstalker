...
stats = {‘cycles’: 0, ‘checked’: 0, ‘alerts’: 0,
‘filter_mc’: 0, ‘filter_liq’: 0, ‘filter_vol’: 0,
‘filter_buy’: 0, ‘filter_age’: 0, ‘filter_score’: 0}
...
@flask_app.route(’/’)
def health():
return {‘status’: ‘alive’, ‘cycles’: stats[‘cycles’], ‘alerts’: stats[‘alerts’]}
...
v = float(v)
if v >= 1000000:
return ‘$’ + str(round(v/1000000, 2)) + ‘M’
if v >= 1000:
...
if not c:
return None
return (time.time() * 1000 - float(c)) / 60000

def pot(score, mc):
if mc < 40000 and score >= 90:
return ‘10x’
if mc < 100000 and score >= 85:
return ‘5x’
return ‘2x’

async def get_pair(address):
try:
r = await http.get(DEX_TOKEN + address)
...
logger.debug(’Pair error: ’ + str(e))
return None

async def rpc_call(method, params):
try:
r = await http.post(SOL_RPC, json={‘jsonrpc’: ‘2.0’, ‘id’: 1, ‘method’: method, ‘params’: params}, timeout=12)
return r.json().get(‘result’)
except Exception as e:
logger.debug(’RPC error: ’ + str(e))
return None

async def check_chain(address):
result = {‘mint_revoked’: True, ‘freeze_revoked’: True, ‘top10_pct’: None, ‘dev_pct’: None, ‘whale_alert’: False}
try:
...
top10_str = str(round(top10, 1)) + ‘%’ if top10 is not None else ‘N/A’
dev_str = str(round(dev, 1)) + ‘%’ if dev is not None else ‘N/A’
age_str = str(round(age)) + ‘m’ if age is not None else ‘N/A’
link = ‘https://dexscreener.com/solana/’ + address
return (
‘<b>GEMSTALKER ALERT</b>\n\n’
‘<b>’ + name + ‘</b> ($’ + symbol + ‘)\n’
’Source: ’ + source + ‘\n\n’
’MC: ’ + fmt(mc) + ’ | Liq: ’ + fmt(liq) + ‘\n’
’Vol: ’ + fmt(vol) + ’ | Age: ’ + age_str + ‘\n’
’Buys: ’ + str(round(buy_pct)) + ‘%\n’
’Mint: ’ + (‘OK’ if mint_ok else ‘NO’) + ’ | Freeze: ’ + (‘OK’ if freeze_ok else ‘NO’) + ‘\n’
’Top10: ’ + top10_str + ’ | Dev: ’ + dev_str + ‘\n\n’
’Score: ’ + str(score) + ‘/100\n’
’Potential: ’ + pot(score, mc) + ‘\n\n’
...
return
score = score_token(vol, liq, buy_pct, change, chain[‘mint_revoked’], chain[‘freeze_revoked’], top10, dev, age, chain[‘whale_alert’])
if score < MIN_SCORE:
stats[‘filter_score’] += 1
return
seen_tokens.add(address)
stats[‘alerts’] += 1
card = build_card(name, symbol, mc, liq, vol, buy_pct, age, chain[‘mint_revoked’], chain[‘freeze_revoked’], top10, dev, score, address, source)
try:
await bot.send_message(chat_id=CHAT_ID, text=card, parse_mode=‘HTML’, disable_web_page_preview=True)
logger.info(‘ALERT: ’ + symbol + ’ score=’ + str(score))
except Exception as e:
logger.error(’Send error: ’ + str(e))
tracked_tokens[address] = {‘name’: name, ‘symbol’: symbol, ‘entry_mc’: mc, ‘alert_time’: time.time(), ‘milestones’: set()}
daily_calls.append({‘address’: address, ‘name’: name, ‘symbol’: symbol, ‘entry_mc’: mc, ‘alert_time’: time.time()})
...
logger.error(’Profiles error: ’ + str(e))
try:
...
logger.error(’Boosts error: ’ + str(e))
try:
...
msg = (’<b>PNL - ’ + label + ’ HIT!</b>\n\n’ + entry[‘name’] + ’ ($’ + entry[‘symbol’] + ‘)\n’
’Entry: ’ + fmt(entry[‘entry_mc’]) + ’\nNow: ’ + fmt(cur_mc) + ‘\n’
’Multiple: ’ + str(round(mult, 2)) + ’x\nTime: ’ + str(elapsed) + ‘m\n\n’
‘<a href="https://dexscreener.com/solana/' + addr + '">Chart</a>’)
await bot.send_message(chat_id=CHAT_ID, text=msg, parse_mode=‘HTML’, disable_web_page_preview=True)
if ‘4x’ in entry[‘milestones’] or (time.time() - entry[‘alert_time’]) > 21600:
tracked_tokens.pop(addr, None)
except Exception as e:
logger.debug(’Track error: ’ + str(e))
...
buy_pct = buys / total * 100 if total > 0 else 0
age = get_age(pair)
score = score_token(vol, liq, buy_pct, change, chain[‘mint_revoked’], chain[‘freeze_revoked’], chain[‘top10_pct’], chain[‘dev_pct’], age, chain[‘whale_alert’])
card = build_card(name, symbol, mc, liq, vol, buy_pct, age, chain[‘mint_revoked’], chain[‘freeze_revoked’], chain[‘top10_pct’], chain[‘dev_pct’], score, address, ‘Manual Scan’)
await reply_fn(card, parse_mode=‘HTML’, disable_web_page_preview=True)

async def start_cmd(update, context):
await update.message.reply_text(‘GemStalker online. Paste any Solana CA to scan.\n\n/scan address\n/calls - today calls\n/status - bot stats’)
...
m, s = divmod(r, 60)
msg = (’GemStalker Status\n\nUptime: ’ + str(h) + ’h ’ + str(m) + ’m ’ + str(s) + ‘s\n’
’Cycles: ’ + str(stats[‘cycles’]) + ’\nChecked: ’ + str(stats[‘checked’]) + ‘\n’
’Alerts: ’ + str(stats[‘alerts’]) + ’\nTracking: ’ + str(len(tracked_tokens)) + ‘\n\n’
’Rejections:\nMC: ’ + str(stats[‘filter_mc’]) + ’\nLiq: ’ + str(stats[‘filter_liq’]) + ‘\n’
’Vol: ’ + str(stats[‘filter_vol’]) + ’\nBuy: ’ + str(stats[‘filter_buy’]) + ‘\n’
’Age: ’ + str(stats[‘filter_age’]) + ’\nScore: ’ + str(stats[‘filter_score’]))
...
mult_str = str(round(mult, 2)) + ‘x’
if mult >= 2:
hit2x += 1
except:
mult_str = ‘N/A’
t = datetime.datetime.fromtimestamp(c[‘alert_time’]).strftime(’%H:%M’)
lines.append(str(i) + ‘. ’ + c[‘name’] + ’ ($’ + c[‘symbol’] + ’) ’ + t + ’ - ’ + mult_str)
await update.message.reply_text(’Calls: ’ + str(len(daily_calls)) + ’ | 2x+: ’ + str(hit2x) + ‘\n\n’ + ‘\n’.join(lines))
